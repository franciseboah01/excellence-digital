...
@section('page_subtitle', 'Organisez vos services par catégories')
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
    ...
    <div class="edc-card p-6">
        <h2 class="text-lg font-bold mb-5" style="color: var(--edc-text-primary);">➕ Ajouter une catégorie</h2>
        <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-4">
            @csrf
            <div>
                <label class="edc-label">Nom *</label>
                <input type="text" name="nom" required class="edc-input" placeholder="Ex : Bureautique">
            </div>
            <div>
    ...
    <div class="lg:col-span-2 edc-card overflow-hidden">
        <div class="px-6 py-4" style="border-bottom: 1px solid var(--edc-border);">
            <h2 class="text-lg font-bold" style="color: var(--edc-text-primary);">📂 {{ $categories->total() }} catégorie(s)</h2>
        </div>
        ...
        <div x-data="{ editOpen: false }" class="transition" style="border-bottom: 1px solid var(--edc-border);"
            ...
            <div class="p-5 flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <p class="font-semibold text-sm" style="color: var(--edc-text-primary);">{{ $categorie->nom }}</p>
                        <p class="text-xs" style="color: var(--edc-text-muted);">{{ $categorie->services_count }} service(s) lié(s)</p>
                    </div>
                </div>
            ...
            <div x-show="editOpen" x-cloak class="px-5 pb-5">
                <form method="POST" action="{{ route('admin.categories.update', $categorie) }}" class="space-y-3">
                    @csrf @method('PUT')
                    <div>
                        <label class="edc-label">Nom</label>
                        <input type="text" name="nom" value="{{ $categorie->nom }}" required class="edc-input">
                    </div>
                    <div>
                        <label class="edc-label">Icône (emoji)</label>
                        <input type="text" name="icone" value="{{ $categorie->icone }}" class="edc-input w-24 text-2xl text-center">
                    </div>
        ...
        @endforelse
        <div class="px-6 py-4" style="border-top: 1px solid var(--edc-border);">{{ $categories->links() }}</div>
    </div>
</div>
@endsection